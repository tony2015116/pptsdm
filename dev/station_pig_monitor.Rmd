...
\`other_monitor\` serves as a complement to two other monitoring functions. It primarily aims to analyze and monitor, based on the CSV data from the past seven days of measurement stations, the daily counts of missing records, extreme weight recordings, total feeding time at each measurement station, total feeding amount per pen per day, and average weight per pen per day.

```{r function-other_monitor}
#' Other feed intake monitor of pig performance test station
#' 
#' @param data A data frame or data table. This is the data to be processed. It should have specific columns depending on the station type.
#' @param house_width A character or numeric value specifying the width of the house number.If numeric, it will be converted to a character string.
#' @param save_path A character string specifying the path where the output PNG files will be saved.
#' 
...
#' @import openxlsx
#' @export
other_monitor <- function(data, house_width, save_path) {
  # Check parameters
  ...
  if (missing(house_width) || is.null(house_width)) stop("house_width cannot be NULL")
  ...
  # Processing data
  data <- unique(data)
  with_responder_na <- responder_na(data)
  with_extreme_time_n <- extreme_time_n(data)
  with_process_data <- process_data(data)
  with_low_feedintake <- low_feedintake(with_process_data)
  with_all_feedintake <- all_feedintake(with_process_data)
  with_number_location <- number_location(with_process_data, house_width)
  with_house_feedintake <- house_feedintake(with_process_data, house_width)
  with_merge_data <- merge_data(with_house_feedintake, with_number_location)
  with_house_weight <- house_weight(with_process_data, house_width)
  ...
  # Save all_monitor list to specified path in .xlsx format
  wb <- createWorkbook()
  names(all_monitor) <- sapply(names(all_monitor), function(name) {
    sheet <- addWorksheet(wb, name)
    writeDataTable(wb, sheet, all_monitor[[name]])
    name # Return name to preserve list structure
  })
  saveWorkbook(wb, file.path(save_path, "other_monitor.xlsx"), overwrite = TRUE)
...
low_feedintake <- function(data) {
  ...
  processed_data <- data[date > lubridate::today() - 7][
...
number_location <- function(data, house_width) {
  ...
  ][date > lubridate::today() - 7 # Step 2: Filter rows where 'date' is within the last 7 days from 2023-11-01
...
house_feedintake <- function(data, house_width) {
  ...
  ][date > lubridate::today() - 7  # Step 2: Filter the data to include only the rows with 'date' within the last 7 days from 2023-11-01.
...
house_weight <- function(data, house_width) {
...
You can use `monitor_schedule` to set when to run the `fid_monitor()`、\`station_monitor()`and`\`other_monitor()\`.
...
monitor_schedule <- function(taskname, schedule, starttime, startdate, rscript_args = NULL, ...) {
  ...
  my_function <- function(csv_path, begin_date, house_width, ...) {
    ...
    pptsdm::other_monitor(data = csv_data, house_width = house_width, ...)
  }
...
monitor_schedule(
  ...
  rscript_args = list(house_width = "1", 
                      ...
                      csv_path = "C:/Users/Dell/Documents/projects/pptsdm_data/test_data",
